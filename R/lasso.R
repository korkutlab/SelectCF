...
#'
#' @details
#' This function, rSHC, which robustifies SHC, is robust to the existence of outliers. 
...
#'
#' @description
#' This function computes gap statistic profile for a series of hyperparameters (wbound's) by running rSHC on both original data and permuted data.
#'
#' @param x data matrix. Rows are samples, while columns are features.
...
#' @param return_all_results should rSHC results for all hyperparameters be returned? If False, only the result for the best hyperparameter is returned. The default is FALSE.
...
#' This function runs rSHC for every hyperparameter, calculates gap statistic, and get the best hyperparamter that corresponds to the maximum gap statistic.
#'
#' @return
#' \item{lassotype}{abbreviated name of lasso type.}
#' \item{lassotype_full}{full name of lasso type.}
...
#' \item{result}{rSHC result. If return_all_results is TRUE, results of all hyperparameters are stored as a list, in which each element is a result of rSHC; otherwise, it only contains the result corresponding to the best hyperparameter.}
#' 
#' @examples
#' d <- sim_data(out_pct = 0.1, iseed = 1)
#' out <- rSHC_gapstat(d$x,
...
#' @export
print.rSHC <- function(x, ...) {
  print.SelectCF_base(x, ...)
}


#' @export
print.rSHC_gapstat <- function(x, ...) {
  print.SelectCF_base_gapstat(x, ...)
}


#' @export
plot.rSHC_gapstat <- function(x, ...) {
  plot.SelectCF_base_gapstat(x, ...)
}
