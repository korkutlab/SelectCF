...
GenSynData<- function(size_clusters,
  ...
  for(i in 1:n_clusters){
    ...
    # inf variables
    X <- mvrnorm(size_clusters[i], mu=MU_clusters[,i], Sigma=S_clusters)
...
#'
#' @param size_clusters a vector containing the cluster sizes to be generated. The length of the vector corresponds to the number of clusters to be generated. The default is c(40, 40, 40).
#' @param p_inf number of informative variables describing the generated cluster structure. The default is 50.
#' @param p_noise number of noise variables which do not contribute to the cluster separation. The default is 950.
#' @param mu numeric number used for generating the mean of multivariate Gaussian distributions of clusters. The default is 2.
#' @param rho_unif_range interval used for generating covariance matrice of multivariate Gaussian distributions of clusters. The default is \code{list(min=0.1, max=0.9)}.
#' @param out_pct proportion of outliers. The default is 0.
#' @param out_p number of variables whose values are replaced by uniformly distributed numbers for outliers. The default is 50.
#' @param out_unif_range interval of uniform distribution for outliers. The default is \code{list(min=-6, max=6)}.
#' @param iseed seed of random number generator. The default is NULL.
#'
#' @details
#' Clusters are generated in the first \code{p_inf} informative variables with various characteristics following Gaussian models.
...
#'
#' @return
#' \item{x}{data matrix of the synthetic dataset.}
#' \item{y}{integer vector corresponding to the cluster membership before outlier contamination.}
#' \item{lb}{integer vector with cluster lables and outlier label (0).}
#'
#' @examples
...
#'
#' # cluster membership with uniformly distributed outliers in 0 cluster
#' table(d$lb)
#'
...
#' @export
#'
sim_data <- function(size_clusters = c(40, 40, 40),
                     p_inf = 50,
                     p_noise = 950,
                     mu = 2,
