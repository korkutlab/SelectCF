...
#'
#' @description
#' Robust Selector of Clustering Features (using spike-and-slab lasso penalty).
#'
...
#'
#' @details
#' This function, rSelectCF, which robustifies SelectCF, is robust to the existence of outliers. 
...
#' 
#' @examples
#' d <- sim_data(out_pct = 0.1, iseed = 1)
#' result <- rSelectCF(d$x,
#'                     lambda0 = 200,
#'                     lambda1 = 0.001)
#' result
#' 
...
#'
#' @description
#' Selector of Clustering Features (using spike-and-slab lasso penalty).
#'
#' @param x data matrix. Rows are samples, while columns are features.
#' @param penalty type of spike-and-slab lasso penalty. This should be either "adaptive" (with a random theta, where theta ~ B(a,b)) or "separable" (with a fixed theta). The default is "adaptive".
#' @param lambda0 spike variance parameter. 
...
#'
#' @details
#' SelectCF employs a spike-and-slab lasso penalty to approach the theoretically ideal feature selection coupled with clustering. 
#'
#' @return
#' \item{lassotype}{abbreviated name of lasso type.}
#' \item{lassotype_full}{full name of lasso type.}
...
#'
#' @description
#' This function computes gap statistic profile for a series of hyperparameters (lambda0's) by running rSelectCF on both original data and permuted data.
#'
#' @param x data matrix. Rows are samples, while columns are features.
#' @param penalty type of spike-and-slab lasso penalty. This should be either "adaptive" (with a random theta, where theta ~ B(a,b)) or "separable" (with a fixed theta). The default is "adaptive".
...
#' @param return_all_results should rSelectCF results for all hyperparameters be returned? If False, only the result for the best hyperparameter is returned. The default is FALSE.
...
#' This function runs rSelectCF for every hyperparameter, calculates gap statistic, and get the best hyperparamter that corresponds to the maximum gap statistic.
#'
#' @return
#' \item{lassotype}{abbreviated name of lasso type.}
#' \item{lassotype_full}{full name of lasso type.}
...
#' \item{result}{rSelectCF result. If return_all_results is TRUE, results of all hyperparameters are stored as a list, in which each element is a result of rSelectCF; otherwise, it only contains the result corresponding to the best hyperparameter.}
#' 
#' @examples
#' d <- sim_data(out_pct = 0.1, iseed = 1)
...
#'
#' @description
#' This function computes gap statistic profile for a series of hyperparameters (lambda0's) by running SelectCF on both original data and permuted data.
#'
#' @param x data matrix. Rows are samples, while columns are features.
#' @param penalty type of spike-and-slab lasso penalty. This should be either "adaptive" (with a random theta, where theta ~ B(a,b)) or "separable" (with a fixed theta). The default is "adaptive".
#' @param lambda0s a vector of spike variance parameters. If Null, lambda0s is assigned as \code{seq(1, max(nrow(x), ncol(x)), length = 100)}. The default is NULL.
...
#' @param silent should progress messages be suppressed? The default is FALSE.
#' @param warm_start “warm start” strategy to accelerate convergence for a series of hyperparameters. Since solutions for similar hyperparameters are close, w is initialized as a solution for a nearby hyperparameter so that its solution can be found relatively quickly. The default is TRUE.
#' @param return_all_results should SelectCF results for all hyperparameters be returned? If False, only the result for the best hyperparameter is returned. The default is FALSE.
#' @param nperms number of permutations. The default is 10.
#' @param sel_rule rule for choosing the best hyperparameter. This should be either "max" (maximum gap statistic) or "1se.rule" (having a low complexity but within 1 standard error of the maximum gap statistic). The default is "max".
#'
#' @details
#' For a hyperparameter, the gap statistic measures the strength of the clustering based on real data with respect to the one based on randomly permuted data that are supposed to have no cluster. 
#' This function runs SelectCF for every hyperparameter, calculates gap statistic, and get the best hyperparamter that corresponds to the maximum gap statistic.
#'
#' @return
#' \item{lassotype}{abbreviated name of lasso type.}
#' \item{lassotype_full}{full name of lasso type.}
...
#' \item{best_hyp}{best hyperparameter (i.e., best lambda0).}
#' \item{best_index}{index of best hyperparameter (i.e., best lambda0).}
#' \item{result}{SelectCF result. If return_all_results is TRUE, results of all hyperparameters are stored as a list, in which each element is a result of SelectCF; otherwise, it only contains the result corresponding to the best hyperparameter.}
#' 
#' @examples
#' d <- sim_data(iseed = 1)
#' out <- SelectCF_gapstat(d$x,
#'                         lambda0s = seq(0.01, 0.5*max(nrow(d$x), ncol(d$x)), length = 20),
#'                         lambda1 = 0.001,
#'                         nperms = 3)
#' plot(out)
  ...
  sel_rule = c("max", "1se.rule"))
{
  ...
  if (return_all_results) {
    ...
    out$result$class_name <- "SelectCF"
  }
  
  class(out) <- "SelectCF_gapstat"
  return(out)
}


#' @export
print.SelectCF <- function(x, ...) {
  print.SelectCF_base(x, ...)
}


#' @export
print.SelectCF_gapstat <- function(x, ...) {
  print.SelectCF_base_gapstat(x, ...)
}


#' @export
plot.SelectCF_gapstat <- function(x, ...) {
  plot.SelectCF_base_gapstat(x, ...)
}


#' @export
print.rSelectCF <- function(x, ...) {
  print.SelectCF_base(x, ...)
}


#' @export
print.rSelectCF_gapstat <- function(x, ...) {
  print.SelectCF_base_gapstat(x, ...)
}


#' @export
plot.rSelectCF_gapstat <- function(x, ...) {
  plot.SelectCF_base_gapstat(x, ...)
}
