...
#'
#' @description
#' Robust Selector of Clustering Features using Fused Lasso penalty.
#'
...
#'
#' @details
#' This function, rSelectCF_FL, which robustifies SelectCF_FL, is robust to the existence of outliers. 
...
#' size_clusters <- c(40, 40, 40)
#' n_clusters <- length(size_clusters)
#' p_inf <- 50
#' p <- 1000
#' p_noise <- p - p_inf
#' 
#' d <- sim_data(size_clusters = size_clusters,
#'               p_inf = p_inf, 
#'               p_noise = p_noise,
#'               out_pct = 0.1,
#'               iseed = 1)
...
#'
#' @description
#' Selector of Clustering Features using Fused Lasso penalty.
#'
#' @param x data matrix. Rows are samples, while columns are features.
#' @param lambda1 sparsity parameter. 
#' @param lambda2 successive difference sparsity parameter. 
...
#'
#' @details
#' SelectCF employs a sparse fused lasso penalty to perform feature selection coupled with clustering. 
  ...
  silent = FALSE)
{
  ...
  result$loop <- NULL
  result$class_name <- "SelectCF_FL"
  
  return(result)
}


#' rSelectCF_FL_gapstat
#'
#' @description
#' This function computes gap statistic profile for a series of hyperparameters (lambda1's and lambda2's) by running rSelectCF_FL on both original data and permuted data.
#'
#' @param x data matrix. Rows are samples, while columns are features.
...
#' @param iseed seed of random numbers used for initializing w. If Null, all elements of w are initialized as a fixed value of 1.0/sqrt(p). Otherwise, the initial w elements are uniformly distributed in \code{[0, 2.0/sqrt(p)]}. The default is NULL.
#' @param silent should progress messages be suppressed? The default is FALSE.
#' @param return_all_results should rSelectCF_FL results for all hyperparameters be returned? If False, only the result for the best hyperparameter is returned. The default is FALSE.
...
#' This function runs rSelectCF_FL for every hyperparameter (i.e., a pair of lambda1 and lambda2), calculates gap statistic, and get the best hyperparamter that corresponds to the maximum gap statistic.
#'
#' @return
#' \item{lassotype}{abbreviated name of lasso type.}
#' \item{lassotype_full}{full name of lasso type.}
...
#' \item{result}{rSelectCF_FL result. If return_all_results is TRUE, results of all hyperparameters are stored as a list, in which each element is a result of rSelectCF_FL; otherwise, it only contains the result corresponding to the best hyperparameter.}
#' 
#' @examples
#' # generate synthetic data
#' size_clusters <- c(40, 40, 40)
  ...
  sel_rule = c("max", "1se.rule"))
{
  ...
  class(out) <- "rSelectCF_FL_gapstat"
  return(out)
}
...
#'
#' @description
#' This function computes gap statistic profile for a series of hyperparameters (lambda1's and lambda2's) by running SelectCF_FL on both original data and permuted data.
#'
#' @param x data matrix. Rows are samples, while columns are features.
#' @param lambda1s sparsity parameters. If Null, lambda1s is assigned as \code{seq(1, max(nrow(x), ncol(x)), length = 10)}. The default is NULL.
#' @param lambda2s successive difference sparsity parameters. If Null, lambda2s is assigned as \code{seq(1, max(nrow(x), ncol(x)), length = 10)}. The default is NULL.
...
#' @param iseed seed of random numbers used for initializing w. If Null, all elements of w are initialized as a fixed value of 1.0/sqrt(p). Otherwise, the initial w elements are uniformly distributed in \code{[0, 2.0/sqrt(p)]}. The default is NULL.
#' @param silent should progress messages be suppressed? The default is FALSE.
#' @param return_all_results should SelectCF_FL results for all hyperparameters be returned? If False, only the result for the best hyperparameter is returned. The default is FALSE.
#' @param nperms number of permutations. The default is 10.
#' @param sel_rule rule for choosing the best hyperparameter. This should be either "max" (maximum gap statistic) or "1se.rule" (having a low complexity but within 1 standard error of the maximum gap statistic). The default is "max".
#'
#' @details
#' For a hyperparameter, the gap statistic measures the strength of the clustering based on real data with respect to the one based on randomly permuted data that are supposed to have no cluster. 
#' This function runs SelectCF_FL for every hyperparameter (i.e., a pair of lambda1 and lambda2), calculates gap statistic, and get the best hyperparamter that corresponds to the maximum gap statistic.
#'
#' @return
#' \item{lassotype}{abbreviated name of lasso type.}
#' \item{lassotype_full}{full name of lasso type.}
...
#' \item{best_hyp2}{best hyperparameter lambda2.}
#' \item{best_index}{index of best hyperparameter (i.e., the optimal pair of lambda1 and lambda2). The pairs are stored in a 1-dimensional vector, with a lambda1-major order.}
#' \item{result}{SelectCF_FL result. If return_all_results is TRUE, results of all hyperparameters are stored as a list, in which each element is a result of SelectCF_FL; otherwise, it only contains the result corresponding to the best hyperparameter.}
#' 
#' @examples
#' # generate synthetic data
#' size_clusters <- c(40, 40, 40)
  ...
  sel_rule = c("max", "1se.rule"))
{
  ...
  if (return_all_results) {
    ...
    out$result$class_name <- "SelectCF_FL"
  }
  
  class(out) <- "SelectCF_FL_gapstat"
  return(out)
}


#' @export
print.SelectCF_FL <- function(x, ...) {
  print.SelectCF_hyp2d(x, ...)
}


#' @export
print.SelectCF_FL_gapstat <- function(x, ...) {
...
#' @export
print.rSelectCF_FL <- function(x, ...) {
  print.SelectCF_hyp2d(x, ...)
}


#' @export
print.rSelectCF_FL_gapstat <- function(x, ...) {
  mat <- round(cbind(x$hyp1s, x$hyp2s, x$w_l0norm, x$gaps_mean, x$gaps_se), 5)
  dimnames(mat) <- list(1:length(x$gaps_mean), c("Hyperparameter1", "Hyperparameter2", "# Non-Zero W's", "Gap Statistic", "Standard Error"))
  print(mat, quote=FALSE)
  cat("Tuning parameter that leads to largest Gap statistic: ", x$best_hyp1, " ", x$best_hyp2, fill=TRUE)
