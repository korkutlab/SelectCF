getuw_lasso_lagrange <- function(ds,
  ...
  # call
  set.seed(iseed)
  res <- SelectCF_getuw_lasso_lagrange(ds,
                                       as.numeric(lambdas),
                                       as.integer(max.iter),
                                       init_random,
                                       silent,
...
#'
#' @description
#' Robust Sparse Hierarchical Clustering via a Lagrange form.
#'
...
#'
#' @details
#' This function, rSHC_lagrange, which robustifies SHC_lagrange, is robust to the existence of outliers. 
...
#' result <- rSHC_lagrange(d$x, lambda = 300)
#' result
#' 
#' @export
#'
rSHC_lagrange <- function(
  x,
  lambda,
...
#' rSHC_lagrange_gapstat
#'
#' @description
#' This function computes gap statistic profile for a series of hyperparameters (lambda's) by running rSHC_lagrange on both original data and permuted data.
#'
#' @param x data matrix. Rows are samples, while columns are features.
...
#' @param return_all_results should rSHC_lagrange results for all hyperparameters be returned? If False, only the result for the best hyperparameter is returned. The default is FALSE.
...
#' This function runs rSHC_lagrange for every hyperparameter, calculates gap statistic, and get the best hyperparamter that corresponds to the maximum gap statistic.
#'
#' @return
#' \item{lassotype}{abbreviated name of lasso type.}
#' \item{lassotype_full}{full name of lasso type.}
...
#' \item{result}{rSHC_lagrange result. If return_all_results is TRUE, results of all hyperparameters are stored as a list, in which each element is a result of rSHC_lagrange; otherwise, it only contains the result corresponding to the best hyperparameter.}
#' 
#' @examples
#' d <- sim_data(out_pct = 0.1, iseed = 1)
#' out <- rSHC_lagrange_gapstat(d$x,
...
#' @export
#'
rSHC_lagrange_gapstat <- function(
  x,
  lambdas = NULL,
  standardize.arrays = FALSE,
  dissimilarity = c("squared.distance",
  ...
  sel_rule = c("max", "1se.rule"))
{
  ...
  class(out) <- "rSHC_lagrange_gapstat"
  return(out)
}
...
#' @export
print.rSHC_lagrange <- function(x, ...) {
  print.SelectCF_base(x, ...)
}


#' @export
print.rSHC_lagrange_gapstat <- function(x, ...) {
  print.SelectCF_base_gapstat(x, ...)
}


#' @export
plot.rSHC_lagrange_gapstat <- function(x, ...) {
  plot.SelectCF_base_gapstat(x, ...)
}
